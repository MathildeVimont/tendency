...
effectVar <- params$effectVar
contrEffect <- params$contrEffect
```
...
varRange <- max(dataSp$year) - min(dataSp$year)
...
plotTrend <- plotGLM(summaryCat = interAnnualSum,  modCat = interAnnualModel,
                     effectVar = effectVar, summary = annualSum, distribution = distribution, 
                     sp = sp, type = "relative", coefs = coefs, contr = contrEffect)

ggsave(filename = paste0(sp, ".png"), plot = plotTrend, 
       path = paste0(resDir, "GLOBAL/TREND"), device = "png")
...
dataLevels <- t(data.frame(nbLevels))
...
saveYearlyVariations(sp = sp, data = dataSp, interestVar = interestVar, interAnnualMod = interAnnualModel,
                     interAnnualSum = interAnnualSum, contr = contrEffect, path = paste0(resDir, "GLOBAL/"))
