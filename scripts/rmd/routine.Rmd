...
interAnnualModel <- makeGLM(data = dataSp, 
                            ...
                            distribution = distribution,
                            zi = zi,
                            contr = c("mean", rep(NULL, length(factorVariables))),
                            nTry = nTry,
                            scaling = scaling,
                            intercept = TRUE)
...
plotTrend <- plotGLM(summaryCat = interAnnualSum,  effectVar = "year",
                     summary = annualSum, distribution = distribution, 
                     sp = sp, type = "relative", coefs = coefs)
...
saveYearlyVariations(sp = sp, data = dataSp, interestVar = interestVar, interAnnualMod = interAnnualModel,
                     interAnnualSum = interAnnualSum, contr = "mean", path = paste0(resDir, "GLOBAL/"))
